const baseUrl = "http:// 172.20.10.3/wheelsOnService_api";
//C:\Windows\scrcpy
const LOGIN_API = "$baseUrl/serviceCenter_login.php";
const SIGNUP_API = "$baseUrl/serviceCenter_signup.php";
const GET_CITIES_API = "$baseUrl/getCities.php";
const LOGOUT_API = "$baseUrl/serviceCenter_logout.php";
const GET_CATEGORIES_API = "$baseUrl/getCategories.php";
const ADD_SERVICE_API = "$baseUrl/addService.php";
const SERVICE_CENTER_ID = "$baseUrl/getServiceCenter_id.php";
const GET_SERVICE_API = "$baseUrl/getServiceCen_Services.php";
const SERVICE_CENTER_DETAILS = "$baseUrl/getServiceCenterDetails.php";
const EDIT_SERVICE_API = "$baseUrl/editService.php";
const EDIT_PROFILE_API = "$baseUrl/editServiceCenterDetails.php";
const GET_BOOKINGS_API = "$baseUrl/get_serviceCenter_bookings.php";
const GET_BOOKING_DETAILS_API = "$baseUrl/get_booking_details.php";
const SERVICE_STATUS_ON = "$baseUrl/serviceStatusOn.php";
const SERVICE_STATUS_OFF = "$baseUrl/serviceStatusOff.php";
